...
public class KieserverClientService {
	...
	public List<DeriveSeason> evaluateSeason(DeriveSeason deriveSeason)
		...
		URL obj = new URL(
				"http://sadie-rules-kieserver-dda-sadie-rules.apps.usae-2.syngentaaws.org:80/services/rest/server/containers/instances/ag-reporting-rules-1");
		...
		if (responseCode == HttpURLConnection.HTTP_OK) { // success
			...
			for (int i = 0; i < jArray.length(); i++) {
				...
				for (int j = 0; j < jArray.length(); j++) {
					...
					deriveSeasonNode.get("com.syngenta.dda.facts.DeriveSeason").toString());
					...
					evaluatedDeriveSeasonList.add(ds);
				}
			}
		} else {
